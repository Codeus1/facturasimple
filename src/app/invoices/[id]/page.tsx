...
const InvoiceFormPage: React.FC<InvoiceFormPageProps> = ({ invoiceId }) => {
            ...
            <>
              ...
              {status === 'PENDING' && (
                <Button
                  ...
                  onClick={form.handleSubmit(d => onSubmit(d, 'PAID'))}
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  type="date"
                  disabled={!isEditable}
                  value={formatDateForInput(form.watch('issueDate'))}
                  ...
                  <TableRow key={field.id}>
                    <TableCell>
                      <Input
                        ...
                        disabled={!isEditable}
                        className="border-transparent focus:border-input px-0 bg-transparent"
                    ...
                    <TableCell>
                      <Input
                        type="number"
                        step="0.01"
                        {...form.register(`items.${index}.priceUnit`, { valueAsNumber: true })}
                        disabled={!isEditable}
                    ...
                    <TableCell className="text-right font-medium">
                      {formatCurrency(
                        (form.watch(`items.${index}.quantity`) || 0) *
                          (form.watch(`items.${index}.priceUnit`) || 0)
                      )}
                    </TableCell>
              ...
              <div className="w-full md:w-1/2 p-4 bg-card rounded-lg border border-border space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    id="irpf"
                    disabled={!isEditable}
                    checked={form.watch('irpfRate') > 0}
                    onChange={e => form.setValue('irpfRate', e.target.checked ? IRPF_RATE : 0)}
                    className="w-4 h-4 rounded border-input text-primary focus:ring-ring"
                  />
                  <Label htmlFor="irpf" className="mb-0 cursor-pointer">
                    Aplicar Retencion IRPF (15%)
                  </Label>
                </div>
              </div>
